    ...
    <nav className="absolute top-0 z-50 flex w-full items-center justify-center text-black transition-all duration-150 ease-in-out md:sticky md:h-24 md:border-b-2 md:border-gray-200 md:bg-white">
      ...
      >
        <div className="flex w-full items-center justify-between bg-white px-6 md:hidden">
          ...
          <Icon
            className={twMerge('inline h-8 w-8')}
            onClick={() => setIsNavbarVisible(!isNavbarVisible)}
          />
        </div>
        <div
          className={`${
            isNavbarVisible ? 'block w-full' : 'hidden'
          } flex flex-col items-center justify-between bg-neutral-500 transition-all duration-300 ease-in-out md:flex md:w-full md:flex-row md:items-center md:justify-between md:bg-transparent`}
        >
          <DropDown
